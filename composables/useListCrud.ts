...
export function useListCrud<T extends Record<string, any>>(
  ...
  const selectItem = async (key: string | number) => {
    ...
    try {
      const response: any = await $fetch(`/api/${apiPath}?${keyField}=${encodeURIComponent(String(key))}`)
  ...
  const setLocalItems = (newItems: any[]) => {
    ...
    if (paginated) {
      ;(data.value as any).data = { ...(data.value as any).data, items: newItems }
    } else {
      ;(data.value as any).data = newItems
    }
  }
