 ...
 *
 * VERİ
 * Yeni bir bölüm modeli açılmadı: istatistikler zaten `Services` modeline
 * bağlıydı, giriş metni için oraya tek bir `statsIntro` alanı eklendi.
 * Yönetimi: Admin > Servisler > "Güven Bandı".
 */
import { computed } from 'vue'

const { data: servicesResponse } = await useFetch('/api/services', {
  // Aynı endpoint'i Hizmetler bölümü de çağırıyor; ortak anahtar sayesinde
  // Nuxt isteği tek sefer yapıp iki bileşende paylaşıyor.
  key: 'services-section',
})

const data = computed(() => servicesResponse.value?.data ?? null)
const intro = computed(() => data.value?.statsIntro || '')
