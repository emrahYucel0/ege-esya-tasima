  ...
  <div class="max-w-4xl mx-auto p-4">
    ...
    <form @submit.prevent="save" class="space-y-8 bg-white p-6 shadow-lg rounded-lg">
      ...
      <div>
        ...
        <input id="copyright" v-model="form.copyright" type="text" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" placeholder="© 2026 Marka Adı — Tüm Hakları Saklıdır." />
      </div>
      ...
      <div class="space-y-4">
        ...
        >
          <div class="flex justify-between items-start mb-2">
            ...
            <button :aria-label="`${index + 1}. Sosyal medya bağlantısını sil`" type="button" @click="removeSocialLink(index)" class="text-white bg-red-500 hover:bg-red-600 rounded-full w-8 h-8 flex items-center justify-center transition duration-150">
      ...
      <div class="space-y-4">
        ...
        >
          <div class="flex justify-between items-start mb-2">
            ...
            <button :aria-label="`${index + 1}. Bölge bağlantısını sil`" type="button" @click="removeRegionLink(index)" class="text-white bg-red-500 hover:bg-red-600 rounded-full w-8 h-8 flex items-center justify-center transition duration-150">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
      ...
      <div class="space-y-4">
        ...
        >
          <div class="flex justify-between items-start mb-2">
            ...
            <button :aria-label="`${index + 1}. Menü bağlantısını sil`" type="button" @click="removeQuickLink(index)" class="text-white bg-red-500 hover:bg-red-600 rounded-full w-8 h-8 flex items-center justify-center transition duration-150">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
      ...
      <div class="space-y-4">
        ...
        >
          <div class="flex justify-between items-start mb-2">
            ...
            <button :aria-label="`${index + 1}. Blog bağlantısını sil`" type="button" @click="removeBlogLink(index)" class="text-white bg-red-500 hover:bg-red-600 rounded-full w-8 h-8 flex items-center justify-center transition duration-150">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
