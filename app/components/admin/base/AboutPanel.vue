...
const { form, message, showDeleteModal, recordId, isSaving, isDeleting, save, remove } = useSectionCrud('about-section', 'about-section', {
  ...
  historyText2: '',
  historyText3: '',
  // seoTitle / seoDescription bilerek YOK — cephede okunmuyorlar, sayfa
  // SEO'su Meta panelinden geliyor (bkz. şablondaki açıklama). Formda
  // bulunmadıkları için kaydetmede undefined gider, Prisma undefined
  // alanları atlar; mevcut veritabanı değerleri silinmez.
  services: [],
  stats: [],
});
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          <div class="flex justify-between items-start mb-2">
            ...
            <button :aria-label="`${index + 1}. İstatistiği sil`" type="button" @click="removeStat(index)" class="text-white bg-red-500 hover:bg-red-600 rounded-full w-8 h-8 flex items-center justify-center transition duration-150">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
