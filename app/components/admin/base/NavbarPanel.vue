      ...
      <div>
        ...
        <div v-for="(link, index) in form.socialLinks" :key="'social-' + index" class="flex items-center space-x-2 mb-2">
          ...
          <input
            ...
            placeholder="URL"
          />
          <!-- Bu butonun GÖRÜNÜR metni "Sil". Etiket onunla BAŞLIYOR
               (WCAG 2.5.3); arkasına sıra bilgisi ekleniyor çünkü yan yana
               duran birkaç "Sil" düğmesi ekran okuyucuda ayırt edilemiyordu. -->
          <button :aria-label="`Sil — ${index + 1}. sosyal medya bağlantısı`" type="button" @click="removeSocialLink(index)" class="px-2 py-1 bg-red-500 text-white rounded">
            Sil
          </button>
        </div>
