<script setup>
const { form, message, showDeleteModal, recordId, isSaving, isDeleting, save, remove } = useSectionCrud('testimonials-section', 'testimonials-section', {
  mainTitle: '',
  subtitle: '',
  tag: '',
  // showDecorations / backgroundColor / textColor bilerek YOK — cephede
  // kullanılmıyorlar (bkz. şablondaki açıklama). Formda bulunmadıkları için
  // kaydetmede undefined gider, Prisma undefined alanları atlar; yani mevcut
  // veritabanı değerleri silinmez.
  testimonials: [],
});
...
<template>
  <div class="max-w-7xl mx-auto p-4">
    ...
    <form @submit.prevent="save" class="space-y-8 bg-white p-6 shadow-lg rounded-lg">
      ...
      <!-- "Görünüm Ayarları" bloğu (Dekoratif Elementleri Göster / Arkaplan
           Rengi / Yazı Rengi) KALDIRILDI. Üç alan da cephede artık
           kullanılmıyor: bölüm design system zeminlerini kullanıyor ve
           dekoratif daireler kaldırıldı (bkz. components/base/Testimonial.vue
           başlığındaki gerekçe). Panelde durdukları sürece "değiştirdim ama
           sitede hiçbir şey olmadı" durumu üretiyorlardı. Veritabanı
           sütunları yerinde bırakıldı; bu form artık o alanları göndermediği
           için mevcut değerler de olduğu gibi korunuyor. -->
        ...
        >
          <div class="flex justify-between items-start mb-4">
            ...
            <div class="flex items-center space-x-2">
              ...
              <button :aria-label="`${testimonialIndex + 1}. Yorumu sil`" type="button" @click="removeTestimonial(testimonialIndex)" class="text-white bg-red-500 hover:bg-red-600 rounded-full w-8 h-8 flex items-center justify-center transition duration-150">
